...
export const FieldToggle = (props: FieldToggleProps) => {
    ...
    const handleDragEnd = (event: React.DragEvent) => {
        ...
        if (dropItem && list && dropItem.closest("ul") === list) {
            if (onMove && selectedItem.dataset.index !== undefined && dropIndex.current !== null) {
    ...
    return (
        ...
        >
            <label htmlFor={`switch_${index}`} className="flex cursor-pointer items-center gap-2">
                <Switch id={`switch_${index}`} checked={selected} onCheckedChange={onToggle} name={`${index}`} />
                <span className="text-sm">
                    <FieldTitle label={label} source={source} resource={resource} />
                </span>
            </label>
            {onMove && <GripVertical className="dragIcon h-4 w-4 cursor-move text-muted-foreground" />}
        </li>
    );
};
