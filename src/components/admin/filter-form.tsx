...
import { type FilterElementProps, useFilterContext } from "@/hooks/filter-context";
import { type SavedQuery, extractValidSavedQueries, useSavedQueries } from "@/hooks/saved-queries";
...
export const FilterFormInput = (inProps: FilterFormInputProps) => {
    ...
    return (
        <div
            ...
            className={cn("filter-field pointer-events-auto relative flex flex-row gap-2", className)}
        >
                    ...
                    {validSavedQueries.map((savedQuery: SavedQuery, index: number) =>
                        isEqual(savedQuery.value, {
                            ...
                            <DropdownMenuItem
                                ...
                                key={savedQuery.label || `saved-query-${index}`}
                            >
                            ...
                            <DropdownMenuItem
                                ...
                                key={savedQuery.label || `query-${index}`}
                            >
...
export const FilterButtonMenuItem = React.forwardRef<HTMLDivElement, FilterButtonMenuItemProps>((props, ref) => {
    ...
    return (
        <div
            className={cn(
                "new-filter-item flex cursor-pointer items-center rounded-sm px-2 py-1.5 text-sm hover:bg-accent",
