...
import { type ArrayInputContextValue, useArrayInput } from "@/hooks/array-input-context";
import {
        ...
        return (
            <SimpleFormIteratorItemContext.Provider value={context}>
                <li
                    ref={ref}
                    className={cn(
                        "flex flex-row items-start justify-between gap-2 border-border border-b pb-2 last:border-b-0",
                        // Align the buttons with the input
                        "[&:has(label)>.simple-form-iterator-item-actions]:pt-10",
                    )}
                >
                    {label != null && label !== false && <p className="mb-2 text-muted-foreground text-sm">{label}</p>}
                    <SourceContextProvider value={sourceContext}>
                        <RecordContextProvider value={record}>
                            <div
                                className={cn("flex flex-1 gap-2", inline ? "flex-col gap-2 sm:flex-row" : "flex-col")}
                            >
                                {children}
                            </div>
                        </RecordContextProvider>
