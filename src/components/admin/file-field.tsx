 ...
 * </div>
 */
export const FileField = <RecordType extends Record<string, unknown> = Record<string, unknown>>(
        ...
        return (
            <ul className={cn("inline-block", className)} {...rest}>
                {sourceValue.map((file, index) => {
                    ...
                    // Generate a more stable key using file properties or fallback to index
                    const key = typeof file === "object" && file && "id" in file ? file.id : `file-${index}`;
                    return (
                        <li key={key}>
                            <a
                                href={srcValue?.toString()}
...
export interface FileFieldProps<RecordType extends Record<string, unknown> = Record<string, unknown>>
