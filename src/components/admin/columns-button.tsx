    ...
    return createPortal(
        <ul className="max-h-[50vh] overflow-auto p-1">
                    ...
                    <div className="relative">
                        ...
                        {columnFilter && (
                            <button
                                ...
                                className="absolute top-2 right-8 h-4 w-4 text-muted-foreground"
                                aria-label="Clear"
                            >
                                ×
                            </button>
            ...
            {paddedColumnRanks.map((position, index) => (
                <DataTableColumnRankContext.Provider value={position} key={`rank-${position}`}>
                    <DataTableColumnFilterContext.Provider value={columnFilter} key={`filter-${position}`}>
...
export const ColumnsSelectorItem = <RecordType extends RaRecord<Identifier> = RaRecord<Identifier>>({
    ...
    const isColumnHidden = source ? hiddenColumns.includes(source) : false;
...
const fieldLabelMatchesFilter = (fieldLabel: string, columnFilter?: string) =>
    columnFilter
        ? diacritic.clean(fieldLabel).toLowerCase().includes(diacritic.clean(columnFilter).toLowerCase())
        : true;
